-- First, let's assign all existing data to the current authenticated user
-- This ensures you don't lose your existing data
UPDATE public.apartments SET user_id = auth.uid() WHERE user_id IS NULL;
UPDATE public.bills SET user_id = auth.uid() WHERE user_id IS NULL;
UPDATE public.rent_payments SET user_id = auth.uid() WHERE user_id IS NULL;
UPDATE public.rooms SET user_id = auth.uid() WHERE user_id IS NULL;
UPDATE public.tenants SET user_id = auth.uid() WHERE user_id IS NULL;
...
-- Create STRICT user-only policies (no admin override)

-- Apartments policies - user can ONLY see their own data
...
-- Bills policies - user can ONLY see their own data
...
-- Rent payments policies - user can ONLY see their own data
...
-- Rooms policies - user can ONLY see their own data
...
-- Tenants policies - user can ONLY see their own data
